...
var htmldiff = {
  ...
  ACTION_NONE: 4,
  ACTION_REPLACE: 5,
  specialCaseOpeningTags: new Array(
    "<strong[\\>\\s]+",
    "<b[\\>\\s]+",
    "<i[\\>\\s]+",
    "<big[\\>\\s]+",
  ...
  Operations: function() {
    ...
    for (var i = 0; i < f.length; i++) {
      ...
      if (b == false && h == false) c = this.ACTION_REPLACE;
      else {
        if (b == true && h == false) c = this.ACTION_INSERT;
        else {
          if (b == false && h == true) c = this.ACTION_DELETE;
          else c = this.ACTION_NONE;
        }
      }
  ...
  IndexNewWords: function(d) {
    ...
    for (var i = 0; i < d.length; i++) {
      var c = d[i];
      if (Array.isArray(b[c])) b[c].push(i);
      else b[c] = [i];
    }
    return b;
  },
  ...
  preg_match_array: function(b, e) {
    var a = 0;
    for (var i = 0; i < b.length; i++) a |= new RegExp(b[i]).test(e);
    return a;
  },
